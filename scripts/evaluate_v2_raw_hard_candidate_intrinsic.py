...
def raw_hard_segments(text: str) -> list[Segment]:
...
def encode_raw_hard_sentencepiece(text: str, processor) -> list[str]:
    logical_tokens: list[str] = []
    for segment in raw_hard_segments(text):
        pieces = processor.EncodeAsPieces(segment.surface)
        for index, piece in enumerate(pieces):
            surface = strip_sp_word_start(piece)
            if not surface:
                continue
            if index == 0 and segment.starts_after_space:
                logical_tokens.append(f"{WORD_START}{surface}")
            else:
                logical_tokens.append(surface)
...
def evaluate_cases_for_models(
    ...
    *,
    ...
    output["protected_hard_raw_sp64"] = []
    ...
    for case in cases:
        ...
        tokens = encode_raw_hard_sentencepiece(case.text, processor)
        output["protected_hard_raw_sp64"].append(
            ModelCaseResult(
                model_name="protected_hard_raw_sp64",
                category=case.category,
                text=case.text,
                expected=case.expected,
                tokens=tokens,
    ...
    *,
    ...
    token_maps = {
        ...
        "protected_hard_raw_sp64": {
            case.text: encode_raw_hard_sentencepiece(case.text, processor)
            for case in cases
        },
    }
    return [
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Evaluate v2 raw-hard candidate intrinsic behavior.")
    parser.add_argument(
        "--candidate-model",
        default="artifacts/private/v2_0_candidate_sentencepiece/protected_hard_raw_sp64_unigram_64000.model",
    )
    parser.add_argument(
        "--sp64-model",
    ...
    parser.add_argument("--report-out", default="artifacts/v2_0_raw_hard_candidate_intrinsic_eval.md")
