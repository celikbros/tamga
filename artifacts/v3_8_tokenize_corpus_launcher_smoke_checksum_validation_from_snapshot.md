# v3.8 Tokenized Corpus Package Checksum Validation

Manifest: `artifacts\tokenizer_v3_0\v3_8_tokenize_corpus_launcher_smoke_20\manifest.json`
Base dir: `C:\CELIK-GARDASH`
Status: `PASS`

## Summary

| Metric | Value |
| --- | ---: |
| schema_version | `v3-production-tokenized-corpus-1` |
| files checked | 5 |
| tokens.bin bytes | 8352 |
| loss_mask.bin bytes | 2088 |
| index.jsonl bytes | 3294 |
| sidecar.jsonl bytes | 11189 |
| manifest.json bytes | 1727 |

## Checksums

| File | SHA-256 |
| --- | --- |
| `tokens.bin` | `5ca6b39d6e5b9bf3047c54281e0376a7982c2ab306724041417612bee320343d` |
| `loss_mask.bin` | `d6d298d9c52a67490472d036a907a1dfa1c4c970443d8fcc0ff5a2e5aa25706d` |
| `index.jsonl` | `997c6c39bbe20a35ff3763e81d76db8846a8a222020e21ed4520bdb5557de53b` |
| `sidecar.jsonl` | `d6b72023f1071b61d86fa0d649ca5753fedee1f5dc61d5affa4097b54495bcc3` |
| `manifest.json` | `50d9dfa195a3e181438852024276d6b660c083fb8c3b8885aa372ca785e3bbb7` |

## Failures

None.

## Warnings

None.
